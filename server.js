...
// Seed users with hashed passwords (Bun.password uses bcrypt by default)
db.prepare('INSERT INTO users (username, password, role) VALUES (?, ?, ?)').run('admin', Bun.password.hashSync('admin-token-xyz'), 'admin');
db.prepare('INSERT INTO users (username, password, role) VALUES (?, ?, ?)').run('alice', Bun.password.hashSync('alice-token-abc'), 'user');
db.prepare('INSERT INTO users (username, password, role) VALUES (?, ?, ?)').run('bob', Bun.password.hashSync('bob-token-def'), 'user');
...
app.post('/login', (req, res) => {
  ...
  // VULNERABLE: String concatenation in SQL query (SQL injection)
